...
var __importD                 efault = (this && this.__importDefault) || function (mod) {
...
var session = conf.session.replace(/Tox-md-WHATSAPP-BOT;;;=>/g, "");
    ...
    try {
        ...
        if (botMsg || baileysMsg) {
           ...
           // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
            const gifLink = "https://raw.githubusercontent.com/NjabuloJ/Tox-md/main/media/remover.gif";
      ...
      await zk.sendMessage(origineMessage, { delete: key });
        ...
        console.log('.... ' + er);
            ...
            if (verifCom) {
                ...
                const cd = evt.cm.find((zokou) => fana.nomCom === (com));
                if (cd) {
                    try {
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ Tox md is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ Tox md Connected to WhatsApp!");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("Toxi md is Online ✅\n\n");
                //chargement des cmds
                console.log("Loading Tox Commands ...\n");
                fs.readdirSync(__dirname + "/cmds").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/cmds/" + fichier);
                            console.log(fichier + " Installed Successfully✔️");
                        }
                        catch (e) {
                            console.log(`${fichier} could not be installed due to : ${e}`);
                        } /* require(__dirname + "/Njabulo Jb/" + fichier);
